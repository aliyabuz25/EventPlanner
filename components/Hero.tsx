           ...
           >
              ...
              <div className="absolute right-0 top-0 sm:top-[-20px] w-[180px] sm:w-[280px] lg:w-[340px] h-[320px] sm:h-[420px] lg:h-[500px] bg-white/20 dark:bg-white/[0.02] backdrop-blur-2xl border border-white/40 dark:border-white/10 rounded-3xl shadow-2xl sm:[transform:translateZ(-60px)] flex flex-col overflow-hidden transition-all duration-700">
                 ...
                 <div className="p-4 sm:p-6 h-full flex flex-col justify-between">
                    <div>
                      <div className="flex items-center justify-between mb-4 sm:mb-8">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[8px] sm:text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em]">Neural Engine</span>
                        </div>
                        <span className="text-[7px] sm:text-[8px] font-mono text-slate-400">v4.2.0</span>
                      </div>
                      <div className="space-y-4 sm:space-y-6">
                        {[
                          ...
                          <div key={item.label}>
                            <div className="flex justify-between text-[8px] sm:text-[9px] font-bold mb-1.5 sm:mb-2 text-slate-700 dark:text-white/80 uppercase tracking-wider">
                      ...
                      <div className="mt-6 sm:mt-10 pt-4 sm:pt-6 border-t border-slate-200/30 dark:border-white/5">
                        <div className="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase mb-2 sm:mb-3 tracking-widest flex items-center justify-between">
                          <span>Active Threads</span>
                          <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-emerald-500 animate-ping"></span>
                        </div>
                        <div className="space-y-1.5 sm:space-y-2 font-mono text-[7px] sm:text-[8px] text-slate-500 dark:text-slate-400 leading-tight">
                    ...
                    <div className="rounded-xl sm:rounded-2xl bg-white/40 dark:bg-white/[0.05] p-3 sm:p-4 border border-white/20 backdrop-blur-md">
                      <div className="flex items-center justify-between mb-1.5 sm:mb-2">
                        <div className="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase">System Status</div>
              ...
              <div className="absolute left-0 top-[60px] sm:top-[40px] w-full sm:w-[500px] lg:w-[620px] h-[300px] sm:h-[420px] lg:h-[480px] bg-white/80 dark:bg-[#050505]/95 backdrop-blur-3xl border border-white dark:border-white/10 rounded-3xl shadow-[0_32px_128px_-32px_rgba(0,0,0,0.25)] dark:shadow-[0_32px_128px_-32px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden sm:[transform:translateZ(80px)] transition-all duration-700">
                 ...
                 <div className="h-12 sm:h-16 bg-white/50 dark:bg-white/[0.02] border-b border-slate-100 dark:border-white/5 flex items-center px-5 sm:px-8 justify-between shrink-0">
                    ...
                    <div className={`flex items-center gap-1.5 sm:gap-2.5 px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-lg sm:rounded-xl text-white shadow-xl transition-all duration-500 scale-90 sm:scale-95 origin-right ${phase === 'result' ? 'bg-emerald-500 shadow-emerald-500/30' : 'bg-sap-blue shadow-sap-blue/30'}`}>
                      ...
                      <span className="text-[8px] sm:text-[10px] font-black uppercase tracking-[0.1em] sm:tracking-[0.15em] shrink-0">
                        {phase === 'typing' ? 'Idle' : phase === 'thinking' ? 'Reasoning' : 'Success'}
                      </span>
                    </div>
                 </div>
                 ...
                 <div className="flex-1 p-5 sm:p-8 flex flex-col gap-5 sm:gap-8 overflow-hidden relative">
                   ...
                   <div className="rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-white/[0.03] p-4 sm:p-5 border border-sap-blue/15 shadow-[inset_0_2px_10px_rgba(0,0,0,0.02)] relative group/prompt overflow-hidden">
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                        ...
                        <span className="text-[8px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">Incoming Strategy</span>
                      </div>
                      <div className="text-[10px] sm:text-[13px] leading-relaxed text-slate-700 dark:text-white/90 font-medium font-mono min-h-[3em]">
                        "{displayText}"
                   ...
                   <div className="grid grid-cols-2 gap-6 sm:gap-10 flex-1 min-h-0">
                     <div className="space-y-4 sm:space-y-5 flex flex-col">
                       <div className="text-[8px] sm:text-[10px] font-black text-sap-accent uppercase tracking-[0.2em] sm:tracking-[0.3em] flex items-center gap-1.5 sm:gap-2">
                         <span className="w-1 h-1 rounded-full bg-sap-accent"></span>
                         Reasoning
                       </div>
                       <div className="flex-1 space-y-3 sm:space-y-4 font-mono text-[8px] sm:text-[10px] leading-tight overflow-hidden">
                         {[
                           { text: 'IDENTIFY_STAKE', resultPhase: 'thinking' },
                           { text: 'SYSTEM_NODES', resultPhase: 'thinking' },
                           { text: 'PHASE_COSTS', resultPhase: 'result' },
                           { text: 'SOW_MODULES', resultPhase: 'result' },
                         ].map((log, i) => {
                           ...
                           return (
                             <div key={i} className={`flex items-center gap-2 sm:gap-3 transition-all duration-500 ${isDone || isCurrent ? 'opacity-100' : 'opacity-20'}`}>
                               <div className={`w-1 sm:w-1.5 h-1 sm:h-1.5 rounded-full ${isDone ? 'bg-emerald-500 brightness-125' : isCurrent ? 'bg-sap-gold animate-pulse' : 'bg-slate-300'}`}></div>
                               <span className="text-slate-600 dark:text-slate-400 truncate">{log.text}</span>
                               <span className={`ml-auto font-black text-[7px] sm:text-[9px] tracking-tighter ${isDone ? 'text-emerald-500' : isCurrent ? 'text-sap-gold' : 'text-slate-400'}`}>
                     ...
                     <div className="space-y-4 sm:space-y-5">
                       <div className="text-[8px] sm:text-[10px] font-black text-sap-gold uppercase tracking-[0.2em] sm:tracking-[0.3em] flex items-center gap-1.5 sm:gap-2">
                         ...
                         {[
                           ...
                           return (
                             <div key={file.name} className={`flex items-center gap-2 sm:gap-4 p-2 sm:p-3 rounded-xl sm:rounded-2xl bg-white dark:bg-white/[0.04] border border-slate-100 dark:border-white/5 transition-all duration-700 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>
                               <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-lg sm:rounded-xl ${file.color} bg-opacity-15 flex items-center justify-center shrink-0`}>
                                 <div className={`w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 ${file.color} rounded shadow-[0_0_10px_${file.color}] opacity-70`}></div>
                               </div>
                               <div className="flex flex-col min-w-0">
                                 <span className="text-[8px] sm:text-[10px] font-black text-slate-800 dark:text-white truncate tracking-tight">{file.name}</span>
                                 <span className="text-[7px] sm:text-[8px] font-mono text-slate-400 uppercase tracking-widest">{file.type}</span>
                               </div>
                             </div>
                           );
                         })}
                   ...
                   <div className="mt-auto pt-5 sm:pt-8 border-t border-slate-100 dark:border-white/5 flex items-center justify-between shrink-0">
                     <div className="flex items-center gap-3 sm:gap-4">
                        ...
                        <div className="hidden sm:block">
                          ...
                          <div className={`text-[10px] font-bold tracking-widest uppercase flex items-center gap-1.5 transition-colors duration-500 ${phase === 'result' ? 'text-emerald-500' : 'text-sap-blue dark:text-sap-accent'}`}>
                            ...
                            {phase === 'typing' ? 'Idle' : phase === 'thinking' ? 'Reasoning' : 'Completed'}
                          </div>
                        </div>
                     </div>
                     <div className="flex flex-col items-end gap-1.5 sm:gap-2.5 bg-slate-50 dark:bg-white/[0.03] px-3 sm:px-4 py-2 sm:py-3 rounded-xl sm:rounded-2xl border border-slate-100 dark:border-white/5">
                       <div className="flex items-baseline gap-1.5 sm:gap-2">
                         <span className="text-[8px] sm:text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Efficiency</span>
